...
#include <ESPAsyncWebServer.h> // Include the HTTP server library
...
void handleGetRequest(AsyncWebServerRequest *request) {
  // Publish the LED state to MQTT when GET request is received
  //client.publish("mynode/led1", String(i));
  
  // Respond to the HTTP request
  String response = "api_key=uktp&sensor=" + version + "&count="+ String(i);
...
void onConnectionEstablished() {
  ...
  server.on("/", HTTP_GET, handleGetRequest);  // Define the route "/publish" for GET requests
